...
function custom_apache_configuration () {    
    ...
    local media_root_alias="Alias ${MEDIA_URL} ${MEDIA_ROOT}"
    ...
    if [ ${USER_NAME} == "www" ]; then
        ...
        echo "<Location ${MEDIA_URL}private/ >" >> "${OSX_SERVER_APACHE_DIR}/${APACHE_CONFIG_FILE}"
        ...
        ised "Alias /files/" "${media_root_alias}" "${OSX_SERVER_APACHE_DIR}/${APACHE_CONFIG_FILE}"
    fi
}

function custom_clean_up () {
    local PRIVATE_FILE_DIR="${MEDIA_ROOT}/private/"
    if [ ! -d ${private_file_dir} ]; then
        mkdir -m 700 "${PRIVATE_FILE_DIR}"
	else
		chmod 700 "${PRIVATE_FILE_DIR}"
	fi
}
...
function install_osx_server_components () {
    ...
    MEDIA_URL=$(python ./manage.py diffsettings  | grep MEDIA_URL  | awk '{print $3}'|sed s/[\'\"]//g)
	...
	if [ "${START_WEBAPP_ON_DEFAULT}" == true ]; then
		webappctl start "${OSX_WEBAPP_PLIST}"
	else
